...
What this version does:
  ...
  • Loads server/floors.json for each floor's corridor_rows and room directory
    (both generated by tools/render_floor_maps.py from the same grid data)
  ...
  • Moves them only through walkable neighboring cells, never through walls
  • Tags each position with the nearest room_id, so backend analytics can
    show foot traffic / dwell-adjacent-to-store without the device ever
    entering the room
  • Publishes grid-aware coordinates (cell centers) so the dashboard can draw
    them directly — see server/floors.json's meters_per_cell (1.0 by default,
    since these are already cell-aligned coordinates, not physical meters)
...
Notes:
  • Set CORRIDOR_ONLY=false to fall back to free movement across every
    walkable cell on the floor (useful for debugging room/grid geometry).
...
# (server/floors.json -> corridor_rows), never entering room interiors. This
# matches real foot traffic: people walk hallways, not through walls into
# rooms they haven't entered. Set to false to fall back to the old behavior
# (free movement across every walkable cell on the floor) for debugging.
...
def load_floors_config() -> dict:
    ...
    Generated by tools/render_floor_maps.py — see that script for how
    corridor_rows is detected and how rooms are assigned.

    Returns {} on any failure (missing file, bad JSON, wrong floor key) so
    the simulator can still start with a clear warning rather than crashing
    — corridor confinement and room tagging simply degrade gracefully.
    """
...
class FloorMap:
    ...
    If corridor_rows is given and CORRIDOR_ONLY is enabled, the walkable mask is
    additionally restricted to that row band — devices then physically cannot
    enter room interiors, only the hallway between them. This is the same band
    tools/render_floor_maps.py highlights on the SVG floor plan, so the dot you
    see on the dashboard and the band rendered on the map are always the same
    source of truth (server/floors.json), not two independently-tuned values.
    """
    ...
    def nearest_room_id(self, col: int) -> str | None:
        """
        Returns the room_id whose column span contains (or is closest to)
        the given column. Used to tag positions with the store the device is
        currently passing — devices never enter the room itself (corridor-only
        confinement), this is purely "which doorway are they nearest to".
        """
        if not self.rooms:
            return None
