...
const TARGET_BUILDING_ID = "cmpwk59uz00000kpagwc05v2s";
...
async function main() {
  ...
  for (const [level, name] of [
    ...
    console.log(
      `Ensured floor level ${level} exists for building ${TARGET_BUILDING_ID}.`,
    );
  }
  ...
  console.log(
    `Identified ${mapNodeToPoiCode.size} MapNodes with POI references to preserve.`,
  );

  // Detach POIs from MapNodes of this building to avoid foreign key restrict violations during deletion
  console.log(
    `Detaching existing POIs from MapNode references for building "${TARGET_BUILDING_ID}"...`,
  );
